...
int collisionDetectionV2(Vector2D start, Vector2D end, TerrainMap circuit) {
	...
	while (nextPoint(&vline, &p, +1) > 0) {
		...
		if (circuit.grid[p.y][p.x] == '1') {
			return 0;
		}
		if (circuit.grid[p.y][p.x] == '2') {
			return 0;
		}
		if (circuit.grid[p.y][p.x] == '3') {
			return 0;
		}
	}
	return 1; 
...
double calulateDistance(TerrainMap map,Vector2D start,Vector2D end){
    if (map.grid[end.y][end.x]=='~'){
        return 9*sqrt((start.x-end.x)*(start.x-end.x)+(start.y-end.y)*(start.y-end.y));
    }
...
GraphList* aStar(GraphNode* start, GraphNode* end, TerrainMap circuit, Vector2D secondPoisition, Vector2D thirdPosition, int gasLevel,
    ...
    initializeStartNode(start, gasLevel, currentSpeed, end);
        ...
        for (accX = -1; accX <= 1; accX++) {
            for (accY = -1; accY <= 1; accY++) {
                ...
                if ((newSpeed.x * newSpeed.x) + (newSpeed.y * newSpeed.y) <= speedMax) {
                    ...
                    if (reachableNode(newPosition, circuit, currentNode->coordinates) == 0) {
                        continue;
                    }
                    ...
                    if (occupied == 1) {
                        ...
                        || !collisionDetection(currentPos, newPos, circuit, secondPoisition, thirdPosition)) {
                            continue;
                        }
                    }

                    if (!collisionDetectionV2(currentPos, newPos, circuit)) {
                        continue;
                    }
                    ...
                    successor->turbo = currentNode->turbo;
...
GraphList* run_a_star(GraphNode* start, GraphNode* end, TerrainMap circuit, Vector2D secondPosition, Vector2D thirdPosition, int gasLevel, Vector2D speed, int vitesse, int flag ,GraphList* path) {
    path = aStar(start, end, circuit, secondPosition, thirdPosition, gasLevel, speed, vitesse, flag);
...
GraphList* find_path(DestinationList endList, GraphNode* start, TerrainMap circuit, Vector2D secondPosition, Vector2D thirdPosition, int gasLevel, Vector2D speed, int flag ,GraphList* path,GraphNode* end){
    ...
    while (i < endList.count && path == NULL) {
        ...
        path = aStar(start, end, circuit, secondPosition, thirdPosition, gasLevel, speed, vitesse, flag);
        run_a_star(start, end, circuit, secondPosition, thirdPosition, gasLevel,speed, vitesse, flag,  path);
        i++;
    }
    return path;
