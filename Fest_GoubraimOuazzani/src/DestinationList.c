    ...
    for (int i = 0; i < map.height; i++) {
        for (int j = 0; j < map.width; j++) {
            if (map.grid[i][j] == '=') {
                ...
                Vector2D speed = {0, 0};
                GraphNode* destination = createGraphNode(destinations.entries[destinations.count - 1].coordinates, speed, 0, 0, start);
                double EuclidienDistance = sqrt(pow((start->coordinates.x - destination->coordinates.x), 2) + pow((start->coordinates.y - destination->coordinates.y), 2));
                destinations.entries[destinations.count - 1].estimatedDistance = EuclidienDistance;
            }
        }
    }
    return destinations;
