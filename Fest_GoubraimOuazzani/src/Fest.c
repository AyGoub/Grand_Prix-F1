...
int main()
{
  ...
  while (!feof(stdin)) {
    ...
    if (myPosition.x == myPreviousPosition.x && myPosition.y == myPreviousPosition.y && round > 1) {
      speed.x = 0;
      speed.y = 0;
    }
    ...
    path = run_a_star(start, end, circuit, secondPosition, thirdPosition, gasLevel, speed, vitesse, flag, path);

    path = find_path(destinationlist, start, circuit, secondPosition, thirdPosition, gasLevel, speed, flag, path, end);
    ...
    if (path == NULL) {
      ...
      GraphNode* firstNode = path->head->next->node;
